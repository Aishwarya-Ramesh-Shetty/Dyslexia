...
## 2.1) Who creates games and questions? (Important)

For this MVP, **games are system-defined content**, not student-generated content.

- Student role only exists in the platform.
- Students **play** games and submit attempts.
- Games/questions should be pre-created by backend seed/content process.
- Frontend should fetch available games from `GET /api/games` and questions from `GET /api/games/:gameId/questions`.

This means the usual runtime flow is:
1. Backend content is seeded once (or updated by developer/content-admin workflow later).
2. Frontend reads game/question data from backend APIs.
3. Frontend never creates game definitions in this MVP.
...
## 10) Seed workflow for predefined games

A starter seed script is included:

- Script: `backend/src/seed/seedGamesAndQuestions.js`
- Command: `npm run seed` (inside `backend/`)

What it does:
- Upserts the 5 required game types.
- Replaces starter questions for each game type.
- Ensures students immediately see games on the frontend without manual DB entry.
