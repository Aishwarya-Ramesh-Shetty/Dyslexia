...
## Step 5: Seed predefined games/questions (recommended first run)

```bash
npm run seed
```

This inserts the 5 default dyslexia games and starter questions so `GET /api/games` returns usable data.
...
## 6) Who creates games? (Important flow clarification)

In this MVP, games are **not created by students from frontend**.

- Backend stores predefined games/questions in DB.
- Frontend only reads them via APIs and renders gameplay.
- Student interactions produce `attempts` records (score/time), which are written by backend.

Flow:
1. Developer seeds content (`npm run seed`)
2. Frontend calls `GET /api/games`
3. Frontend calls `GET /api/games/:gameId/questions`
4. Student plays and submits `POST /api/progress/attempt`
5. Dashboard reads `GET /api/progress/summary`
...
## Issue: `option <name> is not supported`
- Cause: malformed `MONGO_URI` query string (invalid option key).
- Example of wrong URI pattern:
  - `...mongodb.net/?dyslexia&retryWrites=true...`
- Fix:
  - Remove stray query keys
  - Use a valid URI from MongoDB Atlas "Connect" button
  - Ensure `.env` key is exactly `MONGO_URI=...` with no quotes/newlines
